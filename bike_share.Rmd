...
User [data](https://divvy-tripdata.s3.amazonaws.com/index.html) is from a 12 month period from January 2022 - December 2022. (Cyclistic is a fictional company and the dataset has been made publicly available by Motivate International Inc. via license). Riders' personal data is unavailable and prohibited.

-- --

## Data cleaning

Data is cleaned and analyzed using R.
...
```{r datasets, echo=-2, results='hide', message=FALSE}
# load datasets
...
df_2022$day_of_week <- ordered(df_2022$day_of_week,levels=c('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'))
...
Casual riders peak during the weekend while members peak during the weekday. This indicates that members use the bike share services more for commutes while casual riders use the bikes for leisurely purposes on the weekends.
...
Overall, casual riders use the bikes for fewer times but for longer rides while members use the bikes more often for shorter rides. 
...
Most bikes rented are the classic bikes and the electric bikes by both casual riders and members. Members slightly favor the classic bikes and casual riders favor the electric bikes. Members do not ride docked bikes at all while very few casual riders use docked bikes.
...
## Key Findings
  ...
  * Members use the bikes throughout the week but slightly more during the weekdays for their commutes but for shorter rides at a time than casual riders

  * Casual riders use the bikes for longer rides and use it more during the weekends than the weekdays

  * The bikes overall are used more during the warmer summer months compared to the colder winter months with very few casual riders using the bikes during the winter months
...
## Recommendations
  ...
  * Raise prices on single-ride passes or full-day passes. This will entice casual riders who use the service often to become annual members.
